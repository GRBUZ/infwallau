...
console.log('[Upload] Client-side compression enabled - expect 70-90% file size reduction');
  ...
  if (fileInput) {
    fileInput.addEventListener('change', async (e) => {
      ...
      console.log('[Upload] File selected, starting background upload...');

      try {
        // Validation immédiate
        await window.UploadManager.validateFile(file);
        
        // OPTIMISATION: Créer une mini progress bar non-bloquante
        const progressIndicator = document.createElement('div');
        progressIndicator.className = 'upload-progress-mini';
        progressIndicator.style.cssText = `
          position: absolute;
        ...
        `;
        progressIndicator.textContent = 'Uploading image...';
        ...
        if (uploadResult && uploadResult.ok) {
          uploadedImageCache = {
            imageUrl: uploadResult.imageUrl,
            regionId: uploadResult.regionId,
            uploadedAt: Date.now()
          };
          
          progressIndicator.style.background = '#10b981';
          progressIndicator.textContent = '✓ Image ready';
          ...
          console.log('[Upload] Background upload completed:', uploadedImageCache);
        } else {
          throw new Error('Upload failed');
        }
